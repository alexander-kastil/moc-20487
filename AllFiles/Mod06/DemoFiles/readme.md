...
26. In the browser, navigate to the following URL:
    ...
    ```
27. Verify that the response is in the format: **["\***value1**_","_**value2**\*"]**.
...
7. In the **Inbound Port Rules** section , select **Allow selected ports** and provide the following details:
   - In **Select public inbound ports**, select the following: **HTTP(80)**, **HTTPS(443)**, **RDP** and **SSH(22)**.
   - Click **OK**.
...
> **Comment:** If you want to use RDP for further configuration install Xrdp Server after logging on
...
28. To publish the package to the virtual machine, paste the following command:
    ...
    ```
    > **Note**: Change _\<server_IP_address\>_ with the IP address of your virtual machine.
...
> **Comment:** As an alternative you can use `nano default`. Nano Reference can be foud [here](https://wiki.ubuntuusers.de/Nano/)
...
# Lesson 3: Continuous Delivery with Azure DevOps
...
7. Open a new tab in the browser and navigate to **https:/dev.azure.com/**, and then click **Get Azure DevOps free**.
...
> **Comment:** To upload an existing project execute:
...
25. To clone the repository to a local repository, run the following command:
    ...
    ```
    > **Note**: Replace _*youraccount*_ with your user name.
...
15. Replace _{YourInitials}_ with your initials from the **Azure web App**.
16. In **PublishProfiles**, double-click **Staging.pubxml**.
17. Replace _{YourInitials}_ with your initials from the **Azure web App**.
